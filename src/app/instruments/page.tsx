...
import type { Instrument, InstrumentFormData, InstrumentStatus, PaginatedInstruments, Category, Supplier } from '@/types';

const STATUSES: InstrumentStatus[] = ['Operational', 'Under Maintenance', 'Out of Service', 'Retired'];
...
function isOverdue(date: string | null) { return !!date && new Date(date).getTime() < Date.now(); }
...
const emptyForm = (): InstrumentFormData => ({
  ...
  status: 'Operational', location: '', purchase_date: '', purchase_price: '',
  supplier_id: '', warranty_expiry: '', last_calibration: '', next_calibration: '', notes: '',
});

function InstForm({ initial, categories, suppliers, onSubmit, loading }: {
  initial: Instrument | null; categories: Category[]; suppliers: Supplier[];
  onSubmit: (f: InstrumentFormData) => void; loading: boolean;
}) {
  ...
  const set = <K extends keyof InstrumentFormData>(k: K, v: InstrumentFormData[K]) => setForm((f) => ({ ...f, [k]: v }));
  const instCategories = categories.filter((c) => c.type === 'Instruments');
    ...
    <form onSubmit={(e: FormEvent) => { e.preventDefault(); onSubmit(form); }} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2">
          ...
          <input className="input" value={form.name} onChange={(e) => set('name', e.target.value)} required placeholder="e.g., UV-Vis Spectrophotometer" />
        </div>
        <div>
          <label className="label">Model</label>
          <input className="input" value={form.model} onChange={(e) => set('model', e.target.value)} placeholder="e.g., UV-1900i" />
        </div>
        <div>
        ...
        <div>
          ...
          <input className="input" value={form.manufacturer} onChange={(e) => set('manufacturer', e.target.value)} placeholder="e.g., Shimadzu" />
        </div>
        <div>
          <label className="label">Category</label>
          <select className="input" value={form.category_id as string} onChange={(e) => set('category_id', e.target.value)}>
            <option value="">-- None --</option>
            {instCategories.map((c) => <option key={c.id} value={c.id}>{c.parent_name ? `${c.parent_name} › ` : ''}{c.name}</option>)}
          </select>
        </div>
        <div>
          <label className="label">Status</label>
          <select className="input" value={form.status} onChange={(e) => set('status', e.target.value as InstrumentStatus)}>
            {STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
          </select>
        </div>
        <div>
          <label className="label">Location</label>
          <input className="input" value={form.location} onChange={(e) => set('location', e.target.value)} placeholder="e.g., Instrument Room B" />
        </div>
        <div>
      ...
      <div className="flex justify-end pt-2">
        <button type="submit" className="btn-primary" disabled={loading}>{loading ? 'Saving...' : initial ? 'Update' : 'Add Instrument'}</button>
      </div>
    </form>
  );
}
...
export default function InstrumentsPage() {
  ...
  const [editItem, setEditItem] = useState<Instrument | null>(null);
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const r = await api.get<PaginatedInstruments>(`/instruments?${params}`);
  ...
  const handleSave = async (form: InstrumentFormData) => {
    setSaving(true);
    try {
      if (editItem) { await api.put(`/instruments/${editItem.id}`, form); toast.success('Updated'); }
      else { await api.post('/instruments', form); toast.success('Instrument added'); }
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try { await api.delete(`/instruments/${deleteTarget.id}`); toast.success('Deleted'); setDeleteTarget(null); fetchItems(); }
    catch { toast.error('Failed to delete'); }
    finally { setSaving(false); }
  };

  const statusCounts = STATUSES.reduce<Record<string, number>>((a, s) => ({ ...a, [s]: items.filter((i) => i.status === s).length }), {});
  const calibDue = items.filter((i) => isOverdue(i.next_calibration)).length;
  const calibSoon = items.filter((i) => isSoon(i.next_calibration)).length;

  return (
    <AppLayout title="Instruments">
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3 items-center justify-between">
          <div className="flex flex-wrap gap-3 flex-1">
            <input className="input max-w-xs" placeholder="Search instruments..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="divide-y divide-gray-100">
                {loading ? (
                  ...
                  <tr><td colSpan={7} className="text-center py-12 text-gray-400">No instruments found</td></tr>
                ) : items.map((item) => {
                  const calibOverdue = isOverdue(item.next_calibration);
                  const calibSoonFlag = isSoon(item.next_calibration);
                  return (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="table-cell">
                        <span className={`badge ${STATUS_COLORS[item.status as InstrumentStatus] ?? 'bg-gray-100 text-gray-600'}`}>{item.status}</span>
                      </td>
      ...
      <Modal open={formModal} onClose={() => { setFormModal(false); setEditItem(null); }} title={editItem ? 'Edit Instrument' : 'Add Instrument'} size="xl">
        <InstForm initial={editItem} categories={categories} suppliers={suppliers} onSubmit={handleSave} loading={saving} />
      </Modal>
      <ConfirmDialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} title="Delete Instrument" message={`Delete "${deleteTarget?.name}"?`} loading={saving} />
    </AppLayout>
  );
}
